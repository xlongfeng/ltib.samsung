...
rm -rf csr-wifi-bin-${SOC}-%{version}-%{release}
mkdir csr-wifi-bin-${SOC}-%{version}-%{release}
mv $RPM_BUILD_ROOT/%{pfx}/* csr-wifi-bin-${SOC}-%{version}-%{release}
tar -zcf csr-wifi-bin-${SOC}-%{version}-%{release}.tar.gz csr-wifi-bin-${SOC}-%{version}-%{release}
# todo fix this to use rpmdir or something better than TOP:
cp -f csr-wifi-bin-${SOC}-%{version}-%{release}.tar.gz $TOP/rpm/RPMS/"$LINTARCH"
cp -f csr-wifi-bin-${SOC}-%{version}-%{release}.tar.gz $TOP/rpm/SOURCES
